...
- (UIButton *)leftBtn{
    if (!_leftBtn) {
        _leftBtn = [[UIButton alloc] initWithFrame:CGRectMake(100, [UIScreen mainScreen].bounds.size.height - 60,( [UIScreen mainScreen].bounds.size.width - 100 * 2) / 1, 45)];
        _leftBtn.layer.masksToBounds = YES;
        _leftBtn.layer.cornerRadius = 10;
        _leftBtn.alpha = 0;
        _leftBtn.clipsToBounds = YES;
        [_leftBtn setTitleColor:[UIColor colorWithRed:17 / 255.0 green:95 / 255.0 blue:140 / 255.0 alpha:1] forState:UIControlStateNormal];
        _leftBtn.backgroundColor = [UIColor whiteColor];
        [_leftBtn setTitle:@"立即体验" forState:UIControlStateNormal];
    }
    return _leftBtn;
}
- (UIButton *)rightBtn{
    if (!_rightBtn) {
        _rightBtn = [[UIButton alloc] initWithFrame:CGRectMake(( [UIScreen mainScreen].bounds.size.width - 30 * 3) / 2 + 30 * 2, [UIScreen mainScreen].bounds.size.height - 60,( [UIScreen mainScreen].bounds.size.width - 30 * 3) / 2, 50)];
        _rightBtn.layer.masksToBounds = YES;
        _rightBtn.layer.cornerRadius = 4;
        _rightBtn.clipsToBounds = YES;
        _rightBtn.layer.borderWidth = 2;
        _rightBtn.layer.borderColor = [UIColor colorWithRed:252 / 255.0 green:8 / 255.0 blue:65 / 255.0 alpha:1].CGColor;
        [_rightBtn setTitleColor:[UIColor colorWithRed:252 / 255.0 green:8 / 255.0 blue:65 / 255.0 alpha:1] forState:UIControlStateNormal];
        _rightBtn.backgroundColor = [UIColor whiteColor];
        [_rightBtn setTitle:@"登录" forState:UIControlStateNormal];
    }
    return _rightBtn;
}
...
//            [self addSubview:self.rightBtn];
        ...
        // 设置引导页上的页面控制器
        self.imagePageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(DDScreenW*0.0, DDScreenH*0.9 - 50, DDScreenW*1.0, DDScreenH*0.1)];
