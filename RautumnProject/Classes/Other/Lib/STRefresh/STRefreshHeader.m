...
- (void)setupViews {
    ...
    self.colorArray = @[RGBColor(255, 130, 0),
                        RGBColor(255, 130, 0),
                        RGBColor(255, 130, 0)];
    ...
    // image
    self.headerImageV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"loading"]];
    _headerImageV.frame = _contentView.bounds;
    _headerImageV.layer.masksToBounds = YES;
    _headerImageV.layer.cornerRadius = _contentView.bounds.size.width / 2;
...
- (void)beginRefreshingWithCompletionBlock:(void(^)(STRefreshHeader * header))refreshingBlock
{
