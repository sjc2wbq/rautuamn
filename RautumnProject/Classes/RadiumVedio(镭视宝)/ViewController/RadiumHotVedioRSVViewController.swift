...
//
//  RadiumHotVedioRSVViewController.swift
//  RautumnProject
//
...
import UIKit
import SwiftyDrop
class RadiumHotVedioRSVViewController: RadiumBaseVedioRSVViewController {
    ...
    func loadData(){
        if self.page == 1{
            if let model = DBTool.shared.rav(type: 2){
         ...
         let request = RequestProvider.request(api: ParametersAPI.rvUseful(type: 1, newOrHot: 2, pageIndex: page))
        ...
        .subscribe(onNext: { (model) in
            var frames = [RvUsefulFrame]()
            if self.page == 1{
                DBTool.shared.saveRSV(model: model, type: 2)
                model.data.forEach({ (rvUseful) in
                    let frame = RvUsefulFrame()
                        frame.rvUseful = rvUseful
                    frames.append(frame)
