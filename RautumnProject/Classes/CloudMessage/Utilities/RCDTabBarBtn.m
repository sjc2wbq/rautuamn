      ...
      RCConversation *conversation = conversationList[i];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (conversation.unreadMessageCount > 0) {
                TabBarController * tabBarController = (TabBarController *)[UIApplication sharedApplication].keyWindow.rootViewController;
                if (conversation.conversationType == ConversationType_SYSTEM) {
                    NavigationController * navC = (NavigationController *)tabBarController.viewControllers[4];
                    [[NSUserDefaults standardUserDefaults] setValue:nil forKey:@"ConversationType_SYSTEM_Count"];
                    navC.navigationItem.rightBarButtonItem.badgeValue = nil;
                }else{
                    NavigationController * navC = (NavigationController *)tabBarController.viewControllers[0];
                    navC.topViewController.navigationItem.rightBarButtonItem.badgeValue = nil;
                }
