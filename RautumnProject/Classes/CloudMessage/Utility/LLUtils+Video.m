...
//    MBProgressHUD *HUD = [LLUtils showActivityIndicatiorHUDWithTitle:@"准备中..."];
    [MBProgressHUD bwm_showHUDAddedTo:[UIApplication sharedApplication].keyWindow title:@"准备中..."];
...
//            [LLUtils hideHUD:HUD animated:YES];
            ...
            if (finished){
                ...
                UIAlertController * vc = [UIAlertController alertControllerWithTitle:@"示" message:msg preferredStyle:UIAlertControllerStyleAlert];
                [vc addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                    if (cancelCallback) {
                        cancelCallback();
                    }
                }]];
                [vc addAction:[UIAlertAction actionWithTitle:@"发送" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    if (okCallback) {
                        okCallback(mp4Path);
                    }
                ...
                [[self mostFrontViewController] presentViewController:vc animated:YES completion:nil];
            }else {
                [MBProgressHUD bwm_showTitle:@"视频处理失败，已被删除或损坏" toView:[UIApplication sharedApplication].keyWindow hideAfter:1 msgType:BWMMBProgressHUDMsgTypeInfo];
                if (failCallback)
                    failCallback();
            }
        });
