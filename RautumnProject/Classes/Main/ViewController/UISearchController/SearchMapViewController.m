...
@interface SearchMapViewController () <MAMapViewDelegate,MapPoiTableViewDelegate,AMapSearchDelegate,UISearchBarDelegate,SearchResultTableVCDelegate>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(sendLocation)];
...
- (void)initTableView
{
    _tableView = [[MapPoiTableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_mapView.frame)-64, SCREEN_WIDTH, CELL_HEIGHT*CELL_COUNT + 64)];
...
- (void)sendLocation
{
    ...
    if ([self.delegate respondsToSelector:@selector(searchMapViewController:sendLocationWithLatitude:longitude:address:)]) {
        [self.delegate searchMapViewController:self sendLocationWithLatitude:_tableView.selectedPoi.location.latitude longitude:_tableView.selectedPoi.location.longitude address:[NSString stringWithFormat:@"%@",_tableView.selectedPoi.name]];
    }
    //添加Marker标记
