...
//#define RONGCLOUD_IM_APPKEY @"8luwapkv8t6hl" // online key  开发环境
#define RONGCLOUD_IM_APPKEY @"8brlm7uf8jok3" // online key 发布环境
...
static IMHelper *shareInstance = nil;

@interface IMHelper()  <UIApplicationDelegate, RCIMConnectionStatusDelegate,RCIMReceiveMessageDelegate,RCIMUserInfoDataSource,RCIMGroupInfoDataSource>
@end
...
+ (instancetype)shareInstance{
    ...
    dispatch_once(&onceToken, ^{
        shareInstance = [[IMHelper alloc] init];
    });
...
- (void)setUpRongYunIM:(NSDictionary *)launchOptions{
    ...
    [AMapServices sharedServices].apiKey = @"e4d620f82ab01c0bea4ae1f9b92be6a3";
...
//    [self redirectNSlogToDocumentFolder];
    ...
    [[RCIM sharedRCIM] setScheme:@"RautumnRedPacket" forExtensionModule:@"JrmfPacketManager"];
    ...
    [[RCIM sharedRCIM] registerMessageType:RCVideoMessage.class];
    //开启用户信息和群组信息的持久化
...
//    [RCIMClient sharedRCIMClient].logLevel = RC_Log_Level_Info;
    ...
    if (token.length && userId.length && password.length) {
        ...
        [UserModel shared].id = [userId integerValue];
        TabBarController * mainVC = (TabBarController *) [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateInitialViewController];
        ...
        [[RCIM sharedRCIM] connectWithToken:token
                                    success:^(NSString *userId) {
                                        [[NSNotificationCenter defaultCenter] postNotificationName:@"GetUserInfoNotifation" object:nil];
...
- (void)onRCIMReceiveMessage:(RCMessage *)message left:(int)left {
    ...
    UIViewController * tabarC = [UIApplication sharedApplication].keyWindow.rootViewController;
        NSLog(@"message---------%@",message);
    
    if (message.conversationType == ConversationType_SYSTEM) {//系统会话
        if ([message.content isMemberOfClass:[RCCommandMessage class]]) {
            ...
            if ([commandMessage.name isEqualToString:@"establishFriendRelationship"]) { //接收到添加好友的请求
                NSLog(@"接收到添加好友的请求--------%@-",commandMessage.name);
                
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"RCIMReceiveFriendRequestCountNotifation" object:nil];
                    if ([tabarC isKindOfClass:[TabBarController class]]) {
                        TabBarController * vc = (TabBarController *)tabarC;
                        [vc.tabBar showBadgeOnItemIndex:1 badgeValue:0];
                    }
                });
                if ([[NSUserDefaults standardUserDefaults] valueForKey:@"friendRequestCount"] == nil) {
                    [[NSUserDefaults standardUserDefaults] setValue:@(1) forKey:@"friendRequestCount"];

                }else{
                    [[NSUserDefaults standardUserDefaults] setValue:@(1 +  [[[NSUserDefaults standardUserDefaults] valueForKey:@"friendRequestCount"] integerValue]) forKey:@"friendRequestCount"];
                }
                
            }else if ([commandMessage.name isEqualToString:@"friendsAgree"] || [commandMessage.name isEqualToString:@"friendsDelete"]){
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"RefreshAddressBooksNotifation" object:nil];
                });
            }else if ([commandMessage.name isEqualToString:@"userAuthenticationStatus"]){ //认证通过的推送
                [[NSNotificationCenter defaultCenter] postNotificationName:@"UserAuthenticationStatusNotifation" object:nil];

            }
        }else{
            if ([[NSUserDefaults standardUserDefaults] valueForKey:@"ConversationType_SYSTEM_Count"] == nil) {
                [[NSUserDefaults standardUserDefaults] setValue:@(1) forKey:@"ConversationType_SYSTEM_Count"];
                
            }else{
                [[NSUserDefaults standardUserDefaults] setValue:@(1 +  [[[NSUserDefaults standardUserDefaults] valueForKey:@"ConversationType_SYSTEM_Count"] integerValue]) forKey:@"ConversationType_SYSTEM_Count"];
            }
            
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"RCIMConversationTypeSYSTEMNotifation" object:message];
                if ([tabarC isKindOfClass:[TabBarController class]]) {
                    TabBarController * vc = (TabBarController *)tabarC;
                    [vc.tabBar showBadgeOnItemIndex:4 badgeValue:[[[NSUserDefaults standardUserDefaults] valueForKey:@"ConversationType_SYSTEM_Count"] intValue]];
                }
            });
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([tabarC isKindOfClass:[TabBarController class]]) {
                TabBarController * vc = (TabBarController *)tabarC;
                [vc.tabBar showBadgeOnItemIndex:0 badgeValue:[[RCIMClient sharedRCIMClient] getTotalUnreadCount]];
            }
            [[NSNotificationCenter defaultCenter] postNotificationName:@"RCIMReceiveMessageNotifation" object:nil];
        });
    }
    //establishFriendRelationship
    NSLog(@"onRCIMReceiveMessage------%@",message.content.senderUserInfo.name);
    if ([message.content
    ...
    [AFHttpTool getUserInfo:userId
                    success:^(id response) {
                        if ([response[@"code"] intValue] == 200) {
                            ...
                            NSString *portraitUri = [result[@"portraitUri"] stringByReplacingOccurrencesOfString:@"%2F" withString:@"/"];
