...
open class AlertController: UIViewController {
    ...
    let ActionSheetMargin: CGFloat = 0
    ...
    typealias Clourse = () -> Void
    fileprivate var clourse:Clourse?
    ...
    public func touchClourse(aClource: (() -> Void)?) {
        self.clourse = aClource!
    }
    
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if event?.allTouches?.first?.view != self.customView {
            self.clourse?()
        }
    }
